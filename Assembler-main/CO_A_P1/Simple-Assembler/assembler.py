...
for ele in sys.stdin:
     ...
     if(operation1[i-1][0]=='/' and operation1[i-1][1]=='/'):
        continue
     else:
        no_of_line+=1
     ...
     if(operation1[i-1][0]=="var"):
         if(len(operation1[i-1])==2 and var_flag==True ):
            ...
            var_flag=False
            sys.stdout.write(i,"invalid flag")
     

     else:
        if(len(operation1[i-1])==1 and operation1[i-1][0]!="hlt" ):
            if(operation1[i-1][0][-1]!=":"):
        ...
        #errors for type A
            ...
            if(len(operation1[i-1])!=4):
                ...
                exit()
            if(operation1[i-1][1] not in reg1):
                sys.stdout.write("Invalid register name in type A")
                exit()
            
            if(operation1[i-1][2] not in reg1):
                sys.stdout.write("Invalid 1stoperand in type A")
                exit()
            if(operation1[i-1][3] not in reg1 and operation1[i-1][3] not in var):
        ...
        #errors for mov instruction
            ...
            if(len(operation1[i-1])!=3):
                sys.stdout.write("invalid operand in mov instruction")
                exit()
            if(operation1[i-1][1] not in reg1):
            ...
            imediates_value=operation1[i-1][2]
            if(imediates_value[0]==('$')):
                    try:
                       value= int(imediates_value[1:])
                       if not 0 <= value<128:
                            raise ValueError
                    except ValueError:
            ...
            elif((operation1[i-1][2] not in reg1) and (operation1[i-1][2] not in var) ):#
                
                sys.stdout.write("invalid operand for mov")
                exit()
        ...
        #errors for type C
            ...
            if(imediates_value[0]==('$')):
                ...
                sys.stdout.write(i,"Invalid operand for type c")
                exit()
        #errors for type D
            ...
            if(operation1[i-1][1] not in reg1):
                ...
                exit()
            if(operation1[i-1][2] not in labels1 and operation1[i-1][2] not in var):
        ...
        #errors for type E
            ...
            if(len(operation1[i-1])!=2):
                ...
                exit()
            if(operation1[i-1][1] not in labels1 and operation1[i-1][1] not in var):
...
# fucntion for floating point conversion
...
# function for type D
def typeD(value, r1, var1):
    machine_code = operation.get(value)
    i=0
    while i<1:
        machine_code+="0"
        i+=1
    machine_code+=rig.get(r1)+get_binary(variables[var1])
    return  machine_code
